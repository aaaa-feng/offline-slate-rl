...
class OfflineDataCollector:
    ...
    def collect_trajectories_from_model(self, env_name: str, agent, ranker, belief_encoder, 
                                      ...
                                      quality_level: str = "expert") -> SlateDataset:
        """
        使用指定模型收集轨迹数据
        ...
        project_root = Path(__file__).resolve().parent.parent
        item_embeddings_path = project_root / "data" / "embeddings" / env_config['env_embedds']
            ...
            for episode_idx in tqdm(range(num_episodes), desc=f"收集{quality_level}数据"):
                try:
                    trajectory = self._collect_single_episode(
                        ...
                        metrics_calculator, episode_idx, quality_level
                    )
    ...
    def _collect_single_episode(self, environment, agent, ranker, belief_encoder,
                               metrics_calculator, episode_id: int, quality_level: str = "expert") -> Optional[SlateTrajectory]:
        """
        收集单个episode的数据
        ...
        try:
            ...
            while not done and timestep < 100:  # 最大100步
                ...
                observation = SlateObservation(
                    ...
                    raw_obs=None  # 原始obs已经被转换为belief state
                )
                ...
                next_observation = SlateObservation(
                    ...
                    raw_obs=next_obs_raw  # 保存原始观察用于记录
                )
    ...
    def collect_all_diffuse_data(self):
        """
        收集所有diffuse环境的数据
        """
        print("开始收集所有diffuse环境的离线数据...")
        
        # 加载训练好的模型
        print("加载训练好的模型...")
        models = self.model_loader.load_diffuse_models()
        ...
        for env_name in ['diffuse_topdown', 'diffuse_mix', 'diffuse_divpen']:
            ...
            # 收集expert数据
            expert_dataset = self.collect_trajectories_from_model(
                env_name, agent, ranker, belief_encoder, environment,
                self.collection_config['expert']['episodes'], 'expert'
            )
            
            # 保存expert数据
            expert_path = os.path.join(self.output_dir, env_name, 'expert_data.pkl')
            expert_dataset.save(expert_path, format='pickle')
            
            # 保存D4RL格式
            d4rl_path = os.path.join(self.output_dir, env_name, 'expert_data_d4rl.npz')
            expert_dataset.save(d4rl_path, format='d4rl')
            
            print(f"✅ {env_name} expert数据已保存:")
            print(f"  Pickle格式: {expert_path}")
            print(f"  D4RL格式: {d4rl_path}")
            
            # 打印统计信息
            stats = expert_dataset.get_stats()
            print(f"  数据集统计:")
...
def main():
    ...
    # 动态设置默认输出目录
    project_root = Path(__file__).resolve().parent.parent
    default_output_dir = str(project_root / "offline_datasets")
    parser.add_argument('--output_dir', type=str,
                       default=default_output_dir,
                       help='输出目录')
    parser.add_argument('--env_name', type=str, 
                       choices=['diffuse_topdown', 'diffuse_mix', 'diffuse_divpen', 'all'],
                       default='all',
                       help='环境名称')
    ...
    if args.env_name == 'all':
        ...
        # 加载训练好的模型
        print("加载训练好的模型...")
        models = collector.model_loader.load_diffuse_models()
        ...
        try:
            os.chdir(str(project_root))
            environments = collector.env_factory.create_all_diffuse_environments()
        finally:
            os.chdir(original_cwd)
        ...
        environment = environments[args.env_name]

        # 收集expert数据
        expert_dataset = collector.collect_trajectories_from_model(
            args.env_name, agent, ranker, belief_encoder, environment,
            args.episodes, 'expert'
        )
        ...
        expert_path = os.path.join(output_dir, 'expert_data.pkl')
        expert_dataset.save(expert_path, format='pickle')

        # 保存D4RL格式
        d4rl_path = os.path.join(output_dir, 'expert_data_d4rl.npz')
        expert_dataset.save(d4rl_path, format='d4rl')

        print(f"✅ {args.env_name} expert数据已保存:")
        print(f"  Pickle格式: {expert_path}")
        print(f"  D4RL格式: {d4rl_path}")
