...
class ValEpisodeLoop(EvaluationEpochLoop):
    ...
    def on_run_end(self) -> Any:
        ...
        mean_reward = torch.mean(self.cum_rewards).item()
        std_reward = torch.std(self.cum_rewards).item()
        print("\n" + "="*80, flush=True)
        print(f"[VALIDATION @ Step {self.trainer.global_step}]", flush=True)
        print("="*80, flush=True)
        print(f"  Mean Reward:  {mean_reward:.4f} ± {std_reward:.4f}", flush=True)
        print(f"  Min Reward:   {torch.min(self.cum_rewards).item():.4f}", flush=True)
        print(f"  Max Reward:   {torch.max(self.cum_rewards).item():.4f}", flush=True)
        if self.pomdp:
            mean_scores = torch.mean(self.scores).item()
            print(f"  Mean Scores:  {mean_scores:.4f}", flush=True)
        print("="*80 + "\n", flush=True)
        ...
        if self.pomdp:
            ...
            log_plotly_figure(self.trainer.logger, fig, "val_diversity", self.trainer.global_step)
        ...
        if self.pomdp:
            ...
            log_plotly_figure(self.trainer.logger, subfig, "val_scores_rewards", self.trainer.global_step)
        return output
...
class TestEpisodeLoop(EvaluationEpochLoop):
    ...
    def on_run_end(self) -> Any:
        ...
        if self.pomdp:
            mean_scores = torch.mean(self.scores).item()
            print(f"  Mean Scores:  {mean_scores:.4f}", flush=True)
        print(f"  Test Episodes: {self.test_size}", flush=True)
        print("="*80 + "\n", flush=True)
        ...
        if self.pomdp:
            ...
            log_plotly_figure(self.trainer.logger, fig, "test_diversity", self.trainer.global_step)
        ### Plot scores and reward per timestep
        if self.pomdp:
            if self.env.env.rel_threshold is None:
                offset = self.env.env.offset
